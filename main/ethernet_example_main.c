...
void app_main(void)
{
    ...
    if (eth_port_cnt == 1) {
        ...
        // Setting the ESP32 networking properties
        ESP_ERROR_CHECK(esp_netif_dhcpc_stop(eth_netifs[0])); // Stop DHCP client

        esp_netif_ip_info_t ip_info;
        ip_info.ip.addr = esp_ip4addr_aton("192.168.1.2");  // Replace with your desired static IP
        ip_info.gw.addr = esp_ip4addr_aton("192.168.1.1");    // Replace with your gateway
        ip_info.netmask.addr = esp_ip4addr_aton("255.255.255.0"); // Replace with your netmask
        ESP_ERROR_CHECK(esp_netif_set_ip_info(eth_netifs[0], &ip_info)); // Set static IP

        // Printing the current IP address of the device
        esp_netif_ip_info_t check_ip_info;
        ESP_ERROR_CHECK(esp_netif_get_ip_info(eth_netifs[0], &check_ip_info));
        // Prepare to print IP
        char check_ip_str_buffer[50];
        sprintf(check_ip_str_buffer, "%lu", check_ip_info.ip.addr);
        ESP_LOGI(TAG, "Confirming Ethernet is connected: %s", check_ip_str_buffer);      
    } else {
